...
import com.google.oboe.samples.hellooboe.BackgroundRunner;
import com.google.oboe.samples.hellooboe.PlaybackEngine;
import com.phuket.tour.audioplayer.audio_device.AudioDeviceListEntry;
import com.phuket.tour.audioplayer.audio_device.AudioDeviceSpinner;
...
public class OboeActivity extends AppCompatActivity {
    ...
    private TextView mUserInstructionView;
    ...
    private class MyBackgroundRunner extends BackgroundRunner {
        ...
        void handleMessageInBackground(Message message) {
            ...
            Log.i(OboeActivity.TAG, "got background message, what = " + what + ", arg1 = " + arg1);
            int result = 0;
            boolean restart = false;
            switch (what) {
                case WHAT_START:
                    String lplayFilePath = getApplicationContext().getFilesDir().getAbsolutePath() + "/" + playFilePath;
                    result = PlaybackEngine.startEngine2(lplayFilePath, audioApi, deviceId, channelCount);
                    break;
                case WHAT_STOP:
            ...
            if (restart) {
                ...
                String lplayFilePath = getApplicationContext().getFilesDir().getAbsolutePath() + "/" + playFilePath;
                int result2 = PlaybackEngine.startEngine2(lplayFilePath, audioApi, deviceId, channelCount);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_oboe);
        MainActivity.CopyAssets(getApplicationContext(), playFilePath,
                getApplicationContext().getFilesDir().getAbsolutePath(), playFilePath);
        mLatencyText = findViewById(R.id.latencyText);
        mPlayBtn = findViewById(R.id.oboe_play_btn);
        mPlayBtn.setOnClickListener((View v) -> {
            startAudioAsync();
        });
        mStopBtn = findViewById(R.id.oboe_stop_btn);
        mStopBtn.setOnClickListener((View v) -> {
            stopAudioAsync();
        });
        // 启用MpeOboeEngine时，不显示
        // 在jni_bridge.cpp中可以切换HelloOboeEngine和Mp3OboeEngine
        mUserInstructionView = findViewById(R.id.userInstructionView);
        mUserInstructionView.setText(R.string.song_info);
    ...
    protected void showToast(final String message) {
        runOnUiThread(() -> Toast.makeText(OboeActivity.this,
                message,
